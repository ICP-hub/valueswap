...
const MobileNavbar = ({ NavbarData, setClickConnectWallet }) => {
  ...
  useEffect(() => {
    const getDisplayFunction = () => {
      ...
      const FinalId =
        SlicedPrincipal.padEnd(10, '.') + principalString.slice(-3)
  ...
  return (
    <div
      className={`  ${
        isSticky
          ? ' sticky top-0 transition-all delay-75 duration-300 '
          : ' sticky top-[-150px] delay-300 duration-700'
      } z-50 px-4 md:px-8 `}
    >
      <div className='flex justify-center  font-gilroy   '>
        <ul
          className={`md:hidden md:items-center  md:pb-0 pb-12 absolute md:static rounded-lg left-0 w-full md:w-auto md:pl-0  transition-all duration-500 ease-in gap-2 xl:gap-6 ${
            open ? 'top-12 bg-[#010427] md:bg-transparent' : 'top-[-510px]'
          }`}
        >
              ...
              >
                <div className='flex flex-col justify-center text-custom-size-14 sm:leading-10 md:text-xl  items-center'>
                  ...
                  <div
                    className={`${
                      activeLink === index
                        ? ' bg-orange-500 w-full h-[1px] invisible md:visible'
                        : 'w-1 h-[1px] invisible'
                    }`}
                  ></div>
                  <div
                    className={`${
                      activeLink === Link.LinkPath
                        ? ' bg-orange-500 w-full h-[1px] invisible md:visible'
                        : 'w-1 h-[1px] invisible'
                    }`}
                  ></div>
        ...
        <div className='w-full  rounded-2xl  flex justify-between max-w-[1200px]  tracking-wide  items-center md:py-4 px-2'>
          <div className='flex items-center justify-between px-2 md:justify-start'>
            <div className='flex items-center justify-around'>
              <img
                src='./image/valueswap.png'
                alt=''
                className='w-20 h-[1.2rem] sm:w-28 sm:h-[1.7rem]'
              />
          ...
          <div className='w-[30%] md:w-[70%] flex justify-center md:gap-x-4'>
            <div className='text-base flex  gap-4   items-center rounded-b-lg  md:w-[100%] px-2'>
              <ul
                className={`md:flex md:items-center  md:pb-0 pb-12 hidden md:static rounded-lg left-0 w-full md:w-auto md:pl-0  transition-all duration-500 ease-in gap-2 xl:gap-6 ${
                  open
                    ? 'top-12 bg-[#010427] md:bg-transparent'
                    : 'top-[-490px]'
                }`}
              >
                {NavbarData.Links.map((Link, index) => (
                  <li
                    key={index}
                    className='md:ml-2  md:my-0 my-7 font-normal '
                  >
                    <RouterLink
                      to={Link.LinkPath}
                      className='text-white duration-500 hover:text-orange-500'

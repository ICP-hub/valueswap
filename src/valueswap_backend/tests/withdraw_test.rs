...
fn setup() -> (PocketIc, Principal,Principal , Principal) {
    ...
    pic.add_cycles(backend_canister, 50_000_000_000_000); // 2T Cycles
...
#[test]
fn test_withdraw_with_invalid_pool_name() {
    let (pic, backend_canister, ckbtc_canister, cketh_canister) = setup();
    let hardcoded_principal = Principal::from_text("hyhkx-53cuq-lmkqq-yhjmt-eve7b-j5pyf-3evrj-tncch-ilmtl-nrcee-sqe").unwrap();
    ...
    let pool_name = "".to_string(); // Invalid empty pool name
    let amount = Nat::from(1000u128);

    let encoded_args = candid::encode_args((pool_data, pool_name, amount, ckbtc_canister)).unwrap();
    ...
    match response {
        WasmResult::Reply(data) => {
            let result: Result<(), String> = candid::decode_one(&data).unwrap();
            assert!(result.is_err(), "Expected error for invalid pool name");
            assert_eq!(result.unwrap_err(), "Pool name cannot be empty.");
        },
...
fn test_withdraw_with_zero_amount() {
    ...
    let pool_name = "ckbtccketh".to_string();
    let amount = Nat::from(0u128); // Zero amount
    ...
    match response {
        WasmResult::Reply(data) => {
            ...
            assert_eq!(result.unwrap_err(), "Amount to burn must be greater than zero.");
        },
        WasmResult::Reject(message) => panic!("Unexpected reject: {}", message),
    }
}
...
#[test]
fn test_withdraw_with_invalid_pool_data() {
    let (pic, backend_canister, ckbtc_canister, cketh_canister) = setup();
    let hardcoded_principal = Principal::from_text("hyhkx-53cuq-lmkqq-yhjmt-eve7b-j5pyf-3evrj-tncch-ilmtl-nrcee-sqe").unwrap();

    let pool_data = Pool_Data {
        pool_data: vec![], // Empty pool data
        swap_fee: Nat::from(5u128),
    };
    ...
    match response {
        WasmResult::Reply(data) => {
            let result: Result<(), String> = candid::decode_one(&data).unwrap();
            assert!(result.is_err(), "Expected error for invalid pool data");
            assert!(result.unwrap_err().contains("Invalid pool data"));
        },
        WasmResult::Reject(message) => panic!("Unexpected reject: {}", message),
    }
}
...
#[test]
fn test_withdraw_with_insufficient_lp_tokens() {
    let (pic, backend_canister, ckbtc_canister, cketh_canister) = setup();
    let hardcoded_principal = Principal::from_text("hyhkx-53cuq-lmkqq-yhjmt-eve7b-j5pyf-3evrj-tncch-ilmtl-nrcee-sqe").unwrap();

    // First create a pool with some LP tokens
    let pool_data = Pool_Data {
        pool_data: vec![
            CreatePoolParams {
                token_name: "ckbtc".to_string(),
    ...
    let encoded_args = candid::encode_args((pool_data.clone(),)).unwrap();
    ...
    // Now try to withdraw more LP tokens than available
    let pool_name = "ckbtccketh".to_string();
    let amount = Nat::from(1000000000000000u128); // Very large amount
    ...
    match response {
        WasmResult::Reply(data) => {
            let result: Result<(), String> = candid::decode_one(&data).unwrap();
            assert!(result.is_err(), "Expected error for insufficient LP tokens");
            assert!(result.unwrap_err().contains("Insufficient LP tokens"));
        },
        WasmResult::Reject(message) => panic!("Unexpected reject: {}", message),
    }
}
...
#[test]
fn test_withdraw_with_valid_parameters() {
    let (pic, backend_canister, ckbtc_canister, cketh_canister) = setup();
    let hardcoded_principal = Principal::from_text("hyhkx-53cuq-lmkqq-yhjmt-eve7b-j5pyf-3evrj-tncch-ilmtl-nrcee-sqe").unwrap();

    // First approve token transfers for both tokens
    let approval_args = ApproveArgs {
        fee: None,
        memo: None,
        from_subaccount: None,
    ...
    match response {
        WasmResult::Reply(data) => {
            let result: Result<Nat, ApproveError> = candid::decode_one(&data).unwrap();
            assert!(result.is_ok(), "Expected successful CKBTC approval");
        },
        WasmResult::Reject(message) => panic!("Failed to approve CKBTC: {}", message),
    }
    ...
    match response {
        WasmResult::Reply(data) => {
            let result: Result<Nat, ApproveError> = candid::decode_one(&data).unwrap();
            assert!(result.is_ok(), "Expected successful CKETH approval");
        },
        WasmResult::Reject(message) => panic!("Failed to approve CKETH: {}", message),
    }

    // Create a pool with some LP tokens
    let pool_data = Pool_Data {
        pool_data: vec![
            CreatePoolParams {
                token_name: "ckbtc".to_string(),
    ...
    // Now try to withdraw a reasonable amount of LP tokens
    let pool_name = "ckbtccketh".to_string();
    let amount = Nat::from(100000000u128); // Small amount
    ...
    match response {
        WasmResult::Reply(data) => {
            let result: Result<(), String> = candid::decode_one(&data).unwrap();
            assert!(result.is_ok(), "Expected successful LP token burning");
        },
        WasmResult::Reject(message) => panic!("Failed to burn LP tokens: {}", message),
    }
}
